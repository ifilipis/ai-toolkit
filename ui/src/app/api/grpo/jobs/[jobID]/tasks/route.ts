...
export async function GET(request: Request, { params }: { params: { jobID: string } }) {
  ...
  try {
    ...
    return NextResponse.json({
      pairwise: trainerType === DIFFUSION_DPO_TRAINER_TYPE,
...
export async function POST(request: Request, { params }: { params: { jobID: string } }) {
  try {
    ...
    const sampler =
      rawSampler ||
      (trainerType === FLOW_GRPO_TRAINER_TYPE
    ...
    if (trainerType === FLOW_GRPO_TRAINER_TYPE && sampler !== 'flowmatch_step_with_logprob') {
      return NextResponse.json(
        { error: `Unsupported Flow-GRPO sampler '${sampler}'. Supported values: flowmatch_step_with_logprob` },
        { status: 400 },
      );
    }
    if (trainerType === FLOW_GRPO_TRAINER_TYPE && scheduler !== 'flowmatch_step_with_logprob') {
      return NextResponse.json(
